...
Plugin URI: http://oogami.name/project/wpcc/
...
/*
Copyright (C) 2009-2010 Ono Oogami, http://oogami.name/ (ono@oogami.name)
 ...
 * @link https://sagan.me/repository/PROJECT/wp-chinese-conversion/ SVN Repository
 * @link http://wordpress.org/extend/plugins/wp-chinese-conversion/ Plugin Page on wordpress.org, including guides and docs.
 * @link http://oogami.name/project/wpcc/ Plugin Homepage
 *
 */
...
function wpcc_header() {
	...
	echo "\n" . '<!-- WP Chinese Conversion Plugin Version 1.1.10 -->';
...
//]]>
	...
	if( !$wpcc_direct_conversion_flag )
		echo '<script type="text/javascript" src="' . WPCC_ROOT_URL . 'search-variant.js' . '"></script>';
...
function zhconversion($str, $variant = null) {
	...
	if( !empty($wpcc_options['wpcc_no_conversion_tag']) )
		return limit_zhconversion($str, $wpcc_langs[$variant][0], $wpcc_options['wpcc_no_conversion_tag']);
...
/*
function zhconversion2 ($str) {
	global $wpcc_options, $wpcc_langs;
	if( $variant == null ) $variant = $GLOBALS['wpcc_target_lang'];
	if( strpos($str, '__NOCC__') !== false ) return $str;
	if( !empty($wpcc_options['wpcc_no_conversion_tag']) ) 
		return limit_zhconversion($str, $wpcc_langs[$variant][0], $wpcc_options['wpcc_no_conversion_tag'] );
	return $wpcc_langs[$variant][0]($str);
}
...
/**
 * 对输入字符串进行有限中文转换. 不转换$no_conversion_tag这一HTML标签内的中文
 *
 * @param string $str string inputed
 * @param string $function conversion function for current requested chinese language
 * @param string $no_conversion_tag no conversion HTML Tag(s), imploded with '|'. such as 'pre' or 'pre|code'
 * @return converted string
 *
 * 举例说明:  $return = limit_zhconversion($str, 'zhconversion_tw', 'pre');
 *
 * $str == <<< END
 * <html>
 *	 <span>进行繁简转换的中文</span>
 ...
 * END;
 *
 * 可以看到pre这个HTML标签里中文没有繁简转换, 保持原样输出.
 *
 * 如果本插件后台"不转换部分HTML标签里中文"设置不为空, zhconversion函数会调用本函数.
 *
 */
function limit_zhconversion($str, $function, $no_conversion_tag) {
	if( $m = preg_split('/<(' . $no_conversion_tag . ')([^<>]*>)(.*?<\/\1\s*>)/', $str, -1, PREG_SPLIT_DELIM_CAPTURE ) ) {
		$r = '';
		$count = 0;
		foreach($m as $v) {
			$count++;
...
function wpcc_cancel_incorrect_redirect($redirect_to, $redirect_from) {
	global $wp_rewrite;
	if( preg_match('/^.*\/(zh-tw|zh-cn|zh-sg|zh-hant|zh-hans|zh-my|zh-mo|zh-hk|zh|reset)\/?.+$/', $redirect_to) ) {
 ...
 * 否则, 增加一条 '.../zh-tw|zh-hant|...|zh-hans|zh|reset/?$' => 'index.php?var1=$matches[1]..&varN=$matches[N]&variant=$matches[N+1]'的新规则
	...
	if($wpcc_options['wpcc_use_permalink'] == 1) {
		foreach($rules as $key => $value) {
			...
			if( substr($key, -3) == '/?$' ) {
				...
				$rules2[substr($key, 0, -3) . '/(' . $reg . '|zh|reset)/?$'] = $value . '&variant=$matches[' . $number . ']';
			}
		}
	}
		...
		foreach($rules as $key => $value) {
			if( strpos($key, 'trackback') !== false || strpos($key, 'print') !== false || strpos($value, 'lang=') !== false ) continue;
			if( substr($key, -3) == '/?$' ) {
				$rules2['(' . $reg . '|zh|reset)/' . $key] = preg_replace_callback('/\$matches\[(\d+)\]/', '_wpcc_permalink_preg_callback', $value) . '&variant=$matches[1]';
			}
		}
	}
	$rules2['^(' . $reg . '|zh|reset)/?$'] = 'index.php?variant=$matches[1]';//首页的繁简转换版本rewrite规则
...
function wpcc_link_conversion($link, $variant = null) {
	...
	if( strpos($link, '?') !== false || !$wpcc_options['wpcc_use_permalink'] )
		return wp_specialchars(add_query_arg('variant', $variant, $link));
	if($wpcc_options['wpcc_use_permalink'] == 1)
...
function wpcc_get_noconversion_url() {
	...
	$tmp = ( is_ssl() ? 'https://' : 'http://' ) .
		$_SERVER['HTTP_HOST'] .
		( $_SERVER['SERVER_PORT'] != (is_ssl() ? 443 : 80) ? ( ':' . $_SERVER['SERVER_PORT'] ) : '' ) .
		$_SERVER['REQUEST_URI'];
	$tmp = trim(strtolower(remove_query_arg('variant', $tmp)));
	
	if( preg_match('/^(.*)\/(' . $reg . '|zh|reset)(\/.*)?$/', $tmp, $matches) ) {
...
function wpcc_fix_link_conversion($link) {
	...
	if( $wpcc_options['wpcc_use_permalink'] == 1 ) { 
		...
		if(preg_match('/^(.*\/)(zh-tw|zh-cn|zh-sg|zh-hant|zh-hans|zh-my|zh-mo|zh-hk|zh|reset)\/(.+)$/', $link, $tmp))
...
function wpcc_output_navi($args = '') {
	...
	$output .= '	<span id="wpcc_original_link" class="' . ( $wpcc_target_lang == false ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a href="' . $default_url . '" title="' . wp_specialchars($noconverttip) . '">' . wp_specialchars($noconverttip) . '</a></span>' . "\n";

	foreach($wpcc_langs_urls as $key => $value) {
		$tip = !empty($wpcc_options[$wpcc_langs[$key][1]]) ? wp_specialchars($wpcc_options[$wpcc_langs[$key][1]]) : $wpcc_langs[$key][2];
		$output .= '	<span id="wpcc_'. $key . '_link" class="'. ( $wpcc_target_lang == $key ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="'. $value .'" title="' . $tip . '" >' . $tip . '</a></span>' . "\n";
	}
	$output .= '</div>' . "\n";
...
function wpcc_output_navi2() {
	...
	$output = "\n" . '<div id="wpcc_widget_inner">' . "\n";
	$output .= '	<span id="wpcc_original_link" class="' . ( $wpcc_target_lang == false ? 'wpcc_current_lang' : 'wpcc_lang' ) .'" ><a href="' . $default_url . '" title="不轉換">不轉換</a></span>' ."\n";
	$output .= '	<span id="wpcc_cn_link" class="'. ( $wpcc_target_lang == 'zh-cn' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="'. $wpcc_langs_urls['zh-cn'] . '" title="大陆简体" >大陆简体</a></span>' ."\n";
	$output .= '	<span id="wpcc_tw_link" class="'. ( $wpcc_target_lang == 'zh-tw' ? 'wpcc_current_lang' : 'wpcc_lang' ) .'"><a rel="nofollow" href="'. $wpcc_langs_urls['zh-tw'] .'" title="台灣正體" >台灣正體</a></span>' ."\n";
	$output .= '	<span id="wpcc_more_links" class="wpcc_lang" >
	<span id="wpcc_more_links_inner_more" class="'. ( ( $wpcc_target_lang == false || $wpcc_target_lang == 'zh-cn' || $wpcc_target_lang == 'zh-tw' ) ? 'wpcc_lang' : 'wpcc_current_lang' ) . '"><a href="#" onclick="return false;" >其它中文</a></span>
		<span id="wpcc_more_links_inner" >
			<span id="wpcc_hans_link" class="' . ( $wpcc_target_lang == 'zh-hans' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . $wpcc_langs_urls['zh-hans'] . '" title="简体中文" >简体中文' . '</a></span>
			<span id="wpcc_hant_link" class="' . ( $wpcc_target_lang == 'zh-hant' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . $wpcc_langs_urls['zh-hant'] . '" title="繁體中文" >繁體中文' . '</a></span>
			<span id="wpcc_hk_link" class="' . ( $wpcc_target_lang == 'zh-hk' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '"><a rel="nofollow" href="' . $wpcc_langs_urls['zh-hk'] . '" title="港澳繁體" >港澳繁體</a></span>
			<span id="wpcc_sg_link" class="' . ( $wpcc_target_lang == 'zh-sg' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . $wpcc_langs_urls['zh-sg'] . '" title="马新简体" >马新简体</a></span>
		</span>
	</span>';
 ...
 *
 * 使用方法: add_action('posts_where', 'wpcc_filter_search_rule');
...
function wpcc_filter_search_rule($where) {
	...
	foreach( $wp_query->query_vars['search_terms'] as $value ) {
		...
		$original .= "{$and1}(($wpdb->posts.post_title LIKE '%{$value}%') OR ($wpdb->posts.post_content LIKE '%{$value}%'))";
		$valuea = zhconversion_all($value);
		$valuea[] = $value;
			$or .= "{$and1}( ";
			$and2 = '';
			foreach($valuea as $v) {
				$or .= "{$and2}( ". $wpdb->prefix ."posts.post_title LIKE '%" . $v . "%') ";
				$or .= " OR ( ". $wpdb->prefix ."posts.post_content LIKE '%" . $v . "%') ";
				$and2 = ' OR ';
			}
		$or .= ' ) ';
		$and1 = ' AND ';
	...
	$term = $wpdb->escape($wp_query->query_vars['s']);
	if( empty($wp_query->query_vars['sentence']) && count($wp_query->query_vars['search_terms']) > 1 && $wp_query->query_vars['search_terms'][0] != $wp_query->query_vars['s'] ) {
		$original .= " OR ($wpdb->posts.post_title LIKE '%{$term}%') OR ($wpdb->posts.post_content LIKE '%{$term}%')";
		$sa = zhconversion_all($wp_query->query_vars['s']);
		$sa[] = $wp_query->query_vars['s'];
		if(!empty($or)) $and3 = ' OR ';
		else $and3 = '';

		if(!empty($sa)) {
			foreach($sa as $value) {
				$or .= "{$and3}( ". $wpdb->prefix ."posts.post_title LIKE '%" . $wpdb->escape($value) . "%') ";
				$or .= " OR ( ". $wpdb->prefix ."posts.post_content LIKE '%" . $wpdb->escape($value) . "%') ";
				$and3 = ' OR ';
			}
		}
	}
	
	if(empty($or)) return $where;
	$where = preg_replace('/' . preg_quote($original, '/') . '/', $or, $where, 1);
	return $where;
}
 ...
 *
 * @todo 使用Wordpress原生的query var系統讀/寫variant參數, 1.2版本實現. 
...
function wpcc_parse_query($query) {
	...
	if(!$wpcc_target_lang) {
		if( $request_lang == 'zh' ) {
			setcookie('wpcc_variant_' . COOKIEHASH, 'default', time() + 30000000, COOKIEPATH, COOKIE_DOMAIN);
			header('Location: ' . $wpcc_noconversion_url); die();
		}
		if( $request_lang == 'reset' ) {
...
function wpcc_do_conversion() {
	...
	add_filter('the_content', 'zhconversion');
	add_filter('the_title', 'zhconversion' );
	add_filter('the_excerpt', 'zhconversion' );
	...
	add_filter('the_content_rss', 'zhconversion');
	add_filter('the_excerpt_rss', 'zhconversion');
