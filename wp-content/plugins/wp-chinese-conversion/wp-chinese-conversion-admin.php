...
<td>
<input type="text" style="width: 100px;" name="wpcco_no_conversion_tip" id="wpcco_no_conversion_tip" value="<?php echo wp_specialchars($this->options['nctip']); ?>" />
...
<?php foreach($this->langs as $key => $value) { ?>
	...
	<input type="text" style="width: 100px;" name="<?php echo $this->langs[$key][1]; ?>" value="<?php echo !empty($this->options[$value[1]]) ? wp_specialchars($this->options[$value[1]]) : ''; ?>" /><br />
<?php } ?>
<div id="wpcc_help_uls" style="display: none;">未选中的中文语言将不被使用,此项设置为全局设置.您应该选中至少一种中文语言,否则本插件全部功能都不会工作. 在每个复选框后的文本输入框里可以输入该语言自定义名称, 如果留空会使用默认值. ("简体中文", "繁體中文"...)</div>
</td>
</tr>
...
<div id="wpcc_help_sc" style="display: none;">本选项将增强Wordpress搜索功能, 使其对中文关键词繁简统一处理.例如搜索"<code>网络</code>"时, 数据库里含有"<code>网络</code>", "<code >網路</code>" 和"<code >網絡</code>"的文章都会放到搜索结果里返回. 支持多个中文词语搜索, 如搜索<code>"简体 繁体"</code>时, 含有"<code>簡體</code>"和"<code>繁體</code>"两个词的文章也会被返回.(此功能将增加搜索时数据库负担)</div>
</td>
</tr>
...
<div id="wpcc_help_co" style="display: none;"><b>本项设置不会应用于搜索引擎.</b> 如果开启这项设置,本插件将自动保存访客的语言选择.举例而言,当用户通过 "<?php echo $this->options['wpcc_use_permalink'] ? trailingslashit(wpcc_link_conversion(get_option('home') . '/', 'zh-tw')) : wpcc_link_conversion(get_option('home') . '/', 'zh-tw'); ?>" 这个链接访问了你博客的台湾繁体版本时,程序将保存信息到Cookie中. 如果该用户重启浏览器并通过 "<?php echo get_option('home'); ?>/" 再次访问你博客时, 则会被自动跳转到台湾繁体版本的地址. 如果设置为"直接显示对应繁简版本",则不会进行跳转.(参见上一项的说明)<br /><br /><b>本选项和上一项"识别浏览器"选项与缓存插件不兼容</b>.如果您使用了WP Super Cache/ Hyper Cache之类Cache插件, 请把这两项设置均设为"关闭",否则这两个功能不但不会正常工作, 而且可能造成缓存异常.<br /></div>
</td>
</tr>

<tr>
<td valign="top" width="30%"><a title="单击查看帮助" style="cursor: pointer;" onclick="toggleVisibility('wpcc_help_nc');">不转换某些HTML标签里中文:</a></td>
<td>
...
<div id="wpcc_help_nc" style="display: none;">这里输入的HTML标签里内容将不进行中文繁简转换, 保持原样输出.请输入HTML标签名,如<code>pre</code>; 多个HTML标签之间以 | 分割, 如 <code>pre|code</code>. 你可以输入一个不存在的HTML标签, 然后在文章里把不想进行繁简转换内容包含在<code>&lt;!-- &lt;tagname&gt; --&gt;</code>和<code>&lt;!-- &lt;/tagname&gt; --&gt;</code>里.警告:受目前算法限制, 开启此功能可能显著降低页面生成速度. </div>
</td>
</tr>
...
<div id="wpcc_help_pl" style="display: none;">更改此项设置前,<b>请仔细阅读下面的说明:</b><br />本项设置决定插件生成的繁简转换页面Permalink. 默认的形式为您原始Permalink后加上?variant=zh-tw参数.(zh-tw为当前请求的语言代码) .你可以修改这个permalink形式.本插件提供两种非默认的Permalink格式: 您原始的Permalink后加上/zh-tw 或/zh-tw/; 或/zh-tw后加上您原来Permalink. 两种区别在于中文语言代码(zh-tw)附加在您原来Permalink的末尾或开头. URL末尾是否有 / 取决于您的Wordpress永久链接末尾是否有/. 但<b>首页的繁简转换版本URL末尾永远有 "/"</b> . 如果您的Wordpress未开启永久链接,本项设置只能选择第一种URL形式. </div>
</td>
</tr>
...
<div id="wpcc_help_ob" style="display: none;">开启此选项后,插件将对Wordpress输出的全部页面内容进行中文整体转换(使用ob_start和ob_flush函数),这将极大提高页面生成速度并减少资源使用.但也可能造成意想不到问题.如果遇到异常(包括中文转换错误, HTML页面错误或php错误等),请关闭此选项.</div>
</td>
</tr>
...
<div id="wpcc_help_credit" style="display: none;">这个插件是免费和自由的。如果你喜欢本插件，你可以通过这里的选项支持我们。告诉作者我在使用本插件（通过向<a href="http://oogami.name/project/wpcc/" target="_blank" title="插件主页" >插件主页</a>发送一个trackback）；或在博客链接(Links)里添加插件作者的主页（<a href="http://oogami.name/" target="_blank" title="小野大神的 Blog，关注文明与幻想。" >小野大神</a>）。感谢您的支持。</div>
</td>
</tr>
...
<?php
		...
		if($this->admin_lang) {
			...
			$o = limit_zhconversion($o, $this->langs[$this->admin_lang][0], 'pre|code');
		}
		echo $o;
	}
	...
	function process() {
		...
		$options = array(
			...
			'wpcc_no_conversion_tag' => preg_replace('/[^a-z1-9|]/', '', strtolower(trim($_POST['wpcco_no_conversion_tag'], '|')) ),
			'nctip' => $_POST['wpcco_no_conversion_tip'],
		);

		foreach( $this->langs as $lang => $value ) {
			if( !empty( $_POST[$value[1]] ) )
				$options[$value[1]] = $_POST[$value[1]];
		}
		...
		if($options['wpcc_trackback_plugin_author'] == 1) {
			...
			if($options['wpcc_trackback_plugin_author'] == 2)
				$this->message .= '已成功向<a href="http://oogami.name/project/wpcc/" target="_blank">插件主页</a>发送Trackback。感谢您的支持。<br />';
			else
				$this->message .= '向<a href="http://oogami.name/project/wpcc/" target="_blank">插件主页</a>发送Trackback失败，你可以尝试重新提交。<br />';
		}
		
		if($options['wpcc_add_author_link'] == 1 && $this->options['wpcc_add_author_link'] == 0) {
			if($options['wpcc_add_author_link'] = wp_insert_link(array('link_target' =>'_blank', 'link_description' => '小野大神的 Blog，关注文明与幻想。', 'link_rss' => 'http://feed.oogami.name/', 'link_name' => '小野大神', 'link_url' => 'http://oogami.name/')) )
				$this->message .= '已成功添加<a href="http://oogami.name/" target="_blank">插件作者</a>的主页链接。感谢您的支持<br />';
	...
	function trackback() {
		...
		$options['body'] = array(
			...
			'excerpt' => 'Hello, I am using the WP Chinese Conversion Plugin Version ' . $this->version
		);
		$response = @wp_remote_post('http://oogami.name/project/wpcc/trackback/', $options);
		$message = 'WPCC Version ' . $this->version . "\n";
		$message .= get_option('home') . '/';
		@wp_mail('wpcc@oogami.name', 'WPCC Feedback - ' . get_option('blogname'), $message );
