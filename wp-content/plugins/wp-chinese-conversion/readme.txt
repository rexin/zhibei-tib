...
使用方法: 激活插件后到侧边栏添加本插件的Widget即可. 插件将在后台"Options"菜单里增加"Chinese Conversion"子菜单. 你可以到[我的博客](http://oogami.name/ "Demo Link")查看演示, 侧边栏的右上方为本插件输出的Widget效果. (CSS是另加的)

最新稳定版本: Version 1.1.10
...
本插件最新的开发版本并不在Wordpress官方插件库SVN中, 我使用自己的[SVN](https://sagan.me/repository/PROJECT/wp-chinese-conversion/trunk/)更新和维护代码, 而仅在发布稳定版本时commit到官方SVN. 你可以使用下面命令获取本插件最新开发版本:

<code>svn co https://sagan.me/repository/PROJECT/wp-chinese-conversion/trunk/</code>
...
如果使用缓存插件 (WP Super cache, Hyper Cache, etc), 上面部分功能(识别浏览器语言, 使用Cookie保存访客语言偏好)将失效. 在以后的版本里可能加入对缓存插件的支持.
...
== Screenshots ==
Not yet. But you can see demo in [my blog](http://oogami.name/ "Demo Link")
...
III. 这个插件是作者为自己的[Blog](http://oogami.name/ "Author Homepage")写的, 作者有多年独立博客写作历史;
